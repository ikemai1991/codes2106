setwd('D:/myprojects/20210616-substantial revision/immune cells-DEG')
source('D:/myprojects/20210616-substantial revision/immune cells-DEG/myvolcano.R')
source('D:/myprojects/20210616-substantial revision/immune cells-DEG/myGO_KEGG.R')
library(ggplot2)
library(limma)
...
library(ggplot2)
library(limma)
merge_count = read.table('20210616-LIHC-369.txt',header = T,row.names = 1,sep = '\t')
exprSet <- merge_count
...
write.table(DEOutput,'20210617-DEG.txt',sep = '\t')
...
library(ggrepel)
library(dplyr)
myvolcano(df,gene.plot = 10,logfc.cutoff=2,p.cutoff=0.05)
...
#KEGG&GO
upgene = rownames(subset(DEOutput, P.Value < 0.1 & logFC > 2))
downgene = rownames(subset(DEOutput, P.Value < 0.1 & logFC<(-2)))
